...
import { AlertTriangle, ArrowDownUp, FilterX, LayoutDashboard, ListFilter, RotateCcw, Search, Upload, Wrench, MapPin, History, ChevronRight, Info, Building2, Calendar, FileText, User, Euro, CheckCircle2, Edit3, Save, X, AlertCircle, Map as MapIcon, Filter, ArrowUp, ArrowDown, Layers } from "lucide-react";
...
function DashboardTravauxPage() {
  ...
  const filtered = useMemo(() => {
    ...
    // Filtres d'en-tête (Plages de prix, etc.)
  ...
  const dataTranche = useMemo(() => {
    ...
    return Object.entries(map)
      .map(([name, data]) => ({ name, value: data.engage, adresse: data.adresse, pct: (data.engage / total * 100).toFixed(1) }))
  ...
  const historyMap = useMemo(() => {
    ...
    historique.forEach(h => { if (!map.has(h.commande_id)) map.set(h.commande_id, h); });
    return map;
  }, [historique]);
  ...
  return (
    <main className="min-h-screen bg-slate-50/50 pb-12 text-slate-900 font-sans">
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 space-y-6">
        ...
        <section className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            ...
            <div className="space-y-3">
              <Label className="text-[9px] font-black uppercase text-slate-400 tracking-wider">Multi-Sélection Tranches</Label>
              <MultiSelect label="Tranches" options={options.tranches} selected={selectedTranches} onChange={setSelectedTranches} icon={Layers} />
            </div>

            <div className="space-y-3">
              <Label className="text-[9px] font-black uppercase text-slate-400 tracking-wider">Multi-Sélection Villes</Label>
              <MultiSelect label="Villes" options={options.villes} selected={selectedVilles} onChange={setSelectedVilles} icon={MapPin} />
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                {SECTEURS.map(s => (
                  <button key={s} onClick={() => setSelectedSectors(prev => prev.includes(s) ? prev.filter(x => x !== s) : [...prev, s])} className={`px-2 py-1 rounded-lg text-[9px] font-black transition-all border ${selectedSectors.includes(s) ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-slate-50 text-slate-500 border-slate-200'}`}>{s}</button>
                ))}
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                ...
                <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="CMD, ADRESSE..." className="h-9 w-full rounded-xl border border-slate-200 bg-slate-50 pl-9 pr-4 text-[10px] font-black uppercase focus:ring-2 focus:ring-blue-500 outline-none" />
              </div>
            </div>
          </div>
        </section>
        ...
        {/* SECTION CARTE & GRAPHIQUES */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Carte Patrimoniale Épurée */}
          <article className="lg:col-span-2 bg-white rounded-3xl border border-slate-200 p-6 shadow-sm overflow-hidden relative min-h-[400px]">
            <div className="flex items-center justify-between mb-6 relative z-10">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2"><MapIcon className="size-4 text-blue-600" /> Cartographie des Travaux</h3>
              <Badge variant="outline" className="text-[9px] font-black border-slate-200 bg-white/80 backdrop-blur uppercase">Vue Épurée</Badge>
            </div>
            {/* Simulation de carte épurée (SVG ou Placeholder stylisé) */}
            <div className="absolute inset-0 bg-slate-50 flex items-center justify-center opacity-40 select-none">
              <div className="w-full h-full bg-[radial-gradient(#e2e8f0_1px,transparent_1px)] [background-size:20px_20px]" />
            </div>
            <div className="relative z-10 h-full flex items-center justify-center">
              <div className="text-center space-y-4 max-w-sm">
                <div className="inline-flex p-4 rounded-full bg-blue-50 text-blue-600 shadow-xl shadow-blue-100 mb-2"><MapPin className="size-8 animate-bounce" /></div>
                <h4 className="text-sm font-black uppercase tracking-tight text-slate-800">Visualisation Géographique</h4>
                <p className="text-[10px] font-bold text-slate-400 uppercase leading-relaxed">Les marqueurs de travaux sont positionnés sur les villes de {options.villes.map(v => v.label).slice(0, 3).join(', ')}...</p>
                <div className="flex flex-wrap justify-center gap-2 mt-4">
                  {options.villes.slice(0, 8).map(v => (
                    <Badge key={v.value} className="bg-white border-slate-200 text-slate-600 text-[9px] font-black uppercase hover:bg-blue-50 cursor-default transition-colors">{v.label}</Badge>
                  ))}
                </div>
              </div>
            </div>
          ...
          <article className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Répartition Type</h3>
        ...
        {/* Classement Tranches enrichi */}
        <article className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Classement des Tranches (Top Engagé)</h3>
            <Button variant="outline" size="sm" onClick={() => setShowAllTranches(!showAllTranches)} className="text-[9px] font-black uppercase tracking-widest border-slate-200 rounded-xl">
        ...
        {/* JOURNAL DES COMMANDES AVANCÉ */}
        <section className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-5 border-b bg-slate-50/50 flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-900">Journal des Commandes Avancé</h3>
              <Badge className="bg-slate-200 text-slate-700 text-[9px] font-black uppercase tracking-tighter rounded-lg">{filtered.length} LIGNES</Badge>
            </div>
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => setTableFilters({ ...tableFilters, year: { selected: [String(new Date().getFullYear())] } })} className="text-[9px] font-black uppercase tracking-widest border-slate-200 rounded-lg h-8">DERNIÈRE ANNÉE</Button>
            </div>
          </div>
              ...
              <thead className="bg-slate-50 border-b text-slate-400 font-black uppercase tracking-widest">
                <tr>
                  ...
                  <th className="p-4 w-28">
                    <button onClick={() => toggleSort('numero_commande')} className="flex items-center gap-1 hover:text-blue-600 transition-colors">
                      N° Commande {sortConfig?.key === 'numero_commande' && (sortConfig.direction === 'asc' ? <ArrowUp className="size-3" /> : <ArrowDown className="size-3" />)}
                    </button>
                  </th>
                  ...
                  <th className="p-4 w-24">Entreprise</th>
                  <th className="p-4 w-32 text-right">
                    <Popover>
                      <PopoverTrigger asChild>
                        <button className="flex items-center gap-1 ml-auto hover:text-blue-600">Engagé <Filter className="size-3" /></button>
                      </PopoverTrigger>
                      <PopoverContent className="w-64 p-4 rounded-2xl">
                        <Label className="text-[9px] font-black uppercase mb-4 block">Plage Engagé (€)</Label>
                        <div className="space-y-4">
                          <Input type="number" placeholder="Min" onChange={(e) => setTableFilters(p => ({ ...p, engage: { ...p['engage'], min: Number(e.target.value) || undefined } }))} className="h-8 text-[10px] font-black" />
                          <Input type="number" placeholder="Max" onChange={(e) => setTableFilters(p => ({ ...p, engage: { ...p['engage'], max: Number(e.target.value) || undefined } }))} className="h-8 text-[10px] font-black" />
                        </div>
                      </PopoverContent>
                    </Popover>
                  </th>
                  ...
                  return (
                    <tr key={row.id} className="hover:bg-blue-50/30 transition-colors group">
                      ...
                      <td className="p-4 font-black text-blue-600 truncate">
                        <button onClick={() => setSelectedDetail(row)} className="hover:underline flex items-center gap-1">{row.numero_commande}<Info className="size-3 opacity-0 group-hover:opacity-100" /></button>
                      </td>
                      <td className="p-4 font-black text-slate-700 truncate">
                        <Link to="/adresses" search={{ q: "", ville: undefined, tranche: row.tranche_code || undefined, rue: undefined, adresse: undefined }} className="hover:underline">{row.tranche_code || "—"}</Link>
                      </td>
                      ...
                      <td className="p-4">
                        <span className={`px-2 py-0.5 rounded text-[8px] font-black ${sect === 'GT' ? 'bg-blue-100 text-blue-700' : sect === 'GE' ? 'bg-teal-100 text-teal-700' : 'bg-orange-100 text-orange-700'}`}>{sect}</span>
                      </td>
                      ...
                      <td className="p-4 text-center">
                        <div className={`mx-auto size-2 rounded-full ${isProg ? 'bg-green-500 shadow-green-200' : 'bg-slate-300'} shadow-sm`} />
                      </td>
                      <td className="p-4 truncate">
                        <span className="text-slate-400 font-black uppercase text-[8px]">{row.etat_travaux || row.etat_commande || "—"}</span>
                      </td>
                      <td className="p-4 text-center">
                        {modif ? <button onClick={() => { setSelectedModif(modif); setVersionChoice("B"); }} className="text-amber-500 hover:scale-110"><AlertTriangle className="size-4 fill-amber-50" /></button> : <div className="size-4 mx-auto rounded-full border-2 border-slate-100 group-hover:border-slate-200" />}
                      </td>
                    </tr>
                  );
                })}
          ...
          <div className="p-4 bg-slate-50/50 border-t flex items-center justify-between">
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">PAGE {page} SUR {Math.ceil(filtered.length / PAGE_SIZE) || 1}</span>
            <div className="flex gap-2">
      ...
      {/* MODALE FICHE DÉTAILLÉE ÉDITABLE AVEC ID LOT */}
      <Dialog open={!!selectedDetail} onOpenChange={(o) => { if (!o) { setSelectedDetail(null); setIsEditing(false); } }}>
        <DialogContent className="max-w-4xl rounded-3xl p-0 overflow-hidden border-none shadow-2xl">
          <div className="bg-slate-900 p-8 text-white relative">
            <div className="mb-6">
              <h2 className="text-2xl font-black uppercase tracking-tighter mb-1">Fiche Commande #{selectedDetail?.numero_commande}</h2>
              <p className="text-[9px] font-black text-slate-500 uppercase tracking-[0.3em]">Détail exhaustif & Édition Directe</p>
            </div>
            ...
            <div className="bg-slate-800/50 rounded-2xl p-4 mb-8 border border-white/5">
              ...
              {isEditing ? (
                <Textarea value={editForm.descriptif || ""} onChange={(e) => setEditForm(p => ({ ...p, descriptif: e.target.value }))} className="bg-slate-800 border-slate-700 text-xs font-medium text-white min-h-[60px] rounded-xl" />
              ) : (
                <p className="text-xs text-slate-300 leading-relaxed font-medium">{selectedDetail?.descriptif || "Aucun descriptif renseigné."}</p>
              )}
            </div>
            ...
            <div className="flex items-center justify-between mb-8">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8 flex-1">
                <div className="space-y-1">
                  <p className="text-[9px] font-black text-slate-500 uppercase">Type / Secteur</p>
                  {isEditing ? (
                    <select value={editForm.secteur || ""} onChange={(e) => setEditForm(p => ({ ...p, secteur: e.target.value }))} className="bg-slate-800 border-slate-700 text-xs font-black w-full rounded p-1 outline-none">
                      <option value="GT">GT</option><option value="GE">GE</option><option value="CP">CP</option>
                    </select>
                  ) : (
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-black text-blue-400">
                    <Link to="/adresses" search={{ q: "", ville: undefined, tranche: selectedDetail?.tranche_code || undefined, rue: undefined, adresse: undefined }} className="hover:underline">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-black text-teal-400">
                    {selectedDetail?.lot_code ? (
                      <Link to="/adresses" search={{ q: selectedDetail.lot_code, ville: undefined, tranche: undefined, rue: undefined, adresse: undefined }} className="hover:underline flex items-center gap-1">
                ...
                <div className="space-y-1">
                  ...
                  {isEditing ? (
                    <Input value={editForm.etat_travaux || ""} onChange={(e) => setEditForm(p => ({ ...p, etat_travaux: e.target.value }))} className="bg-slate-800 border-slate-700 text-xs font-black h-7 rounded-lg" />
                  ) : (
              ...
              <div className="ml-8">
                {!isEditing ? (
                  <Button onClick={handleEditStart} variant="outline" className="bg-white/10 border-white/20 hover:bg-white/20 text-white font-black text-[10px] uppercase tracking-widest rounded-xl"><Edit3 className="size-3.5 mr-2" /> MODIFIER</Button>
                ) : (
                  <div className="flex gap-2">
                    <Button onClick={handleSave} className="bg-blue-600 hover:bg-blue-500 text-white font-black text-[10px] uppercase tracking-widest rounded-xl"><Save className="size-3.5 mr-2" /> SAUVER</Button>
                    <Button onClick={() => setIsEditing(false)} variant="ghost" className="text-white font-black text-[10px] uppercase tracking-widest rounded-xl">ANNULER</Button>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-12 bg-white">
            <div className="space-y-8">
              <section>
                <h3 className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-400 mb-4"><Building2 className="size-4 text-blue-600" /> Localisation & Nature</h3>
                <div className="space-y-4">
                  <div className="flex justify-between border-b border-slate-50 pb-2">
                    <span className="text-[10px] font-bold text-slate-400 uppercase">Adresse</span>
                    {isEditing ? <Input value={editForm.adresse || ""} onChange={(e) => setEditForm(p => ({ ...p, adresse: e.target.value }))} className="text-xs font-black h-7 w-48 rounded-lg" /> : <span className="text-xs font-black text-slate-700">{selectedDetail?.adresse || "—"}</span>}
                  </div>
                  <div className="flex justify-between border-b border-slate-50 pb-2">
                    <span className="text-[10px] font-bold text-slate-400 uppercase">Corps d'état</span>
                    {isEditing ? <Input value={editForm.corps_etat || ""} onChange={(e) => setEditForm(p => ({ ...p, corps_etat: e.target.value }))} className="text-xs font-black h-7 w-48 rounded-lg" /> : <span className="text-xs font-black text-slate-700">{selectedDetail?.corps_etat || "—"}</span>}
                  </div>
                </div>
              </section>

              <section>
                <h3 className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-400 mb-4"><User className="size-4 text-blue-600" /> Intervenants</h3>
                <div className="space-y-4">
                  <div className="flex justify-between border-b border-slate-50 pb-2">
                    <span className="text-[10px] font-bold text-slate-400 uppercase">Entreprise</span>
                    {isEditing ? (
                      <div className="flex gap-2"><Input value={editForm.fournisseur || ""} onChange={(e) => setEditForm(p => ({ ...p, fournisseur: e.target.value }))} className="text-xs font-black h-7 w-32 rounded-lg" /><Input value={editForm.numero_fournisseur || ""} onChange={(e) => setEditForm(p => ({ ...p, numero_fournisseur: e.target.value }))} className="text-xs font-black h-7 w-16 rounded-lg" /></div>
            ...
            <div className="space-y-8">
              <section>
                <h3 className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-400 mb-4"><Euro className="size-4 text-blue-600" /> Finance & Calendrier</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-50 p-4 rounded-2xl">
                    <p className="text-[9px] font-black text-slate-400 uppercase">Engagé</p>
                    {isEditing ? <Input type="number" value={editForm.engage || 0} onChange={(e) => setEditForm(p => ({ ...p, engage: Number(e.target.value) }))} className="text-xs font-black h-7 rounded-lg" /> : <p className="text-lg font-black text-blue-700">{money(selectedDetail?.engage)}</p>}
                  </div>
                  <div className="bg-slate-50 p-4 rounded-2xl">
                    <p className="text-[9px] font-black text-slate-400 uppercase">Payé</p>
                    {isEditing ? <Input type="number" value={editForm.paye || 0} onChange={(e) => setEditForm(p => ({ ...p, paye: Number(e.target.value) }))} className="text-xs font-black h-7 rounded-lg" /> : <p className="text-lg font-black">{money(selectedDetail?.paye)}</p>}
                  </div>
                </div>
              </section>
            </div>
          ...
          <DialogFooter className="p-6 bg-white border-t">
            <Button onClick={() => { setSelectedDetail(null); setIsEditing(false); }} className="w-full bg-slate-900 font-black text-[10px] uppercase tracking-[0.2em] rounded-2xl h-12">FERMER LA FICHE</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!selectedImportErrors} onOpenChange={(o) => !o && setSelectedImportErrors(null)}>
        <DialogContent className="max-w-2xl rounded-3xl border-none shadow-2xl">
          <DialogHeader>
            <DialogTitle className="text-lg font-black uppercase tracking-tight flex items-center gap-2 text-red-600"><AlertCircle className="size-5" /> Analyse des Erreurs d'Import</DialogTitle>
            <DialogDescription className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Fichier : {selectedImportErrors?.fichier}</DialogDescription>
          </DialogHeader>
          <div className="py-4 space-y-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
            <div className="bg-red-50 p-5 rounded-2xl border border-red-100 text-xs text-red-800 font-bold uppercase tracking-tight leading-relaxed">
              {selectedImportErrors?.erreurs} lignes ont été rejetées car le numéro de commande était manquant ou les données financières étaient corrompues.
            </div>
            <div className="grid grid-cols-2 gap-4">
